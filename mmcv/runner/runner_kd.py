...
class Runner_kd(object):
    ...
    def train(self, data_loader, **kwargs):
        ...
        cocoFirst = True if 'coco-imagenet' in kwargs.get('kd_cfg').type else False
        
        if 'decay-cos' in kwargs.get('kd_cfg').type:
            # coefficient decay in cosine
            from math import cos, pi
            kd_decay = 0.5 * cos(pi * self._epoch / self._max_epochs) + 0.5
        elif 'decay-up-down' in kwargs.get('kd_cfg').type:
            # coefficient first go up then go down
            turning_point = self._max_epochs // 4
            if self._epoch <= turning_point:
                kd_decay = 1 + self._epoch / turning_point
            else:
                kd_decay = 2 - 2 * (self._epoch - turning_point) / (self._max_epochs - turning_point)
        else:
            # coefficient annealing
            kd_decay = 1 - self._epoch / self._max_epochs
        print('kd_decay rate: ', kd_decay)
        
        for i, data_batch in enumerate(data_loader):
            self._inner_iter = i
            self.call_hook('before_train_iter')
            # warm up during kd training, the results w/ or w/o warm up are similar.
            kd_warm = {'hint': False, 'head-cls': False, 'head-reg': False, 'rpn-cls': False, 'rpn-reg': False}
