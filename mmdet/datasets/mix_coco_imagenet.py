...
class MixCocoImageDataset(CustomDataset):
    ...
    def load_annotations_imagenet(self, ann_file_imagenet):
        ...
        if 'imagenet_train.txt' in ann_file_imagenet:
            bbox_file = ann_file_imagenet.replace('imagenet_train.txt', 'imagenet_train.bbox.json')
        elif 'unlabeled2017.json' in ann_file_imagenet:
            self.img_ids_imagenet = mmcv.load(ann_file_imagenet)['images']
            bbox_file = ann_file_imagenet.replace('unlabeled2017.json', 'unlabeled2017.bbox.json')
        elif 'imagenet_selected_118k.txt' in ann_file_imagenet:
            bbox_file = ann_file_imagenet.replace('imagenet_selected_118k.txt', 'imagenet_selected_118k.bbox.json')
        else:
            raise NotImplementedError('No annotations info.')

        if os.path.isfile(bbox_file):
            json_file = open(bbox_file).read()
            self.ann_info_imagenet = json.loads(json_file)
        else:
            self.ann_info_imagenet = []

        if 'unlabeled2017' in ann_file_imagenet:
            # coco unlabeled2017 json
            for i, info in enumerate(self.img_ids_imagenet):
                filename, height, width = info['file_name'], info['height'], info['width']
            ...
            self.info_dict_imagenet = dict()
            for i in range(len(self.ann_info_imagenet)):
                filename = self.ann_info_imagenet[i]['image_id']
                if filename in self.info_dict_imagenet.keys():
                    self.info_dict_imagenet[filename].append(i)
                else:
                    self.info_dict_imagenet[filename] = [i]
        else:
    ...
    def get_ann_info_imagenet(self, idx):
        filename = self.data_infos_imagenet[idx]['filename']
        ann_info_imagenet = []
        if filename in self.info_dict_imagenet.keys():
            for i in self.info_dict_imagenet[filename]:
                ann_info_imagenet.append(self.ann_info_imagenet[i])
        return self._parse_ann_info_imagenet(self.data_infos_imagenet[idx], ann_info_imagenet)
    ...
    def prepare_train_img_imagenet(self, idx):
        ...
        ann_info = self.get_ann_info_imagenet(idx)
        results = dict(img_info=img_info, ann_info=ann_info)
        if self.proposals is not None:
            results['proposals'] = self.proposals[idx]
        self.pre_pipeline_imagenet(results)
        return self.pipeline(results)
    ...
    def _parse_ann_info_imagenet(self, img_info, ann_info):
        gt_bboxes = []
        gt_labels = []
        if ann_info:
            for i, ann in enumerate(ann_info):
                ...
                if score > self.train_thr:
                    bbox = [x1, y1, x1 + w, y1 + h]
                    gt_bboxes.append(bbox)
                    gt_labels.append(self.cat2label_imagenet[ann['category_id']])
        if gt_bboxes:
            gt_bboxes = np.array(gt_bboxes, dtype=np.float32)
            gt_labels = np.array(gt_labels, dtype=np.int64)
        else:
